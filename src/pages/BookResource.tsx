...
const BookResource = () => {
  ...
  const handleViewAvailableResources = async () => {
    ...
    try {
      ...
      const startTime = new Date(selectedSlot.start_time).toTimeString().substring(0, 5);
      const endTime = new Date(selectedSlot.end_time).toTimeString().substring(0, 5);
      
      const resources = await backend.getAvailableResources(dateString, startTime, endTime);
